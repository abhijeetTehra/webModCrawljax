...
async function extractXpath(htmlString, filters) {
...
async function getXpathFull(element) {
  const paths = [];
...
const getNodeFromHtmlString= async(htmlString, xpathquery)=>{
  ...
  return xpath.select(xpathquery, dom);
}

module.exports = { getNodeFromHtmlString, getXpathFull, extractXpath }